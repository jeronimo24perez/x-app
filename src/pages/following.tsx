...
import { getMyFollows, type follow } from "../state/follows";
...
const Following = () => {
    ...
    useEffect(()=>{
       ...
       dispatch(getMyFollows(id || "")).unwrap().then((followed) => {
        const fetchPromises = followed.find.map((item: follow) =>
            fetch(`https://x-backend-ruddy.vercel.app/user/${item.followedId}`)
            .then(res => res.json())
            .then(data => data.user)
        );
